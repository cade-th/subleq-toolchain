...
func emuTest(name string, v interface{}) bool {
	data := v.(map[string]interface{})
	programData := data["program"].([]interface{})
	var program []int
	for _, val := range programData {
		program = append(program, int(val.(float64)))
	}
...
func main() {
	testData, err := tests.LoadJSON("emu_test_data.json")
	if err != nil {
		fmt.Printf("Could not load emu test data: %v\n", err)
		os.Exit(1)
	}

	tests.Run("Emulator Tests", testData, emuTest)
}
